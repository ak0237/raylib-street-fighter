...
void onCrounching(personagens* personagem){
    personagem->pushbox.height /= 2;
}
...
void crounch(personagens* personagem, int state, int substate){
    chstates(personagem, state, substate);
}
...
void inputIdle(personagens* personagem, int qual){
    ...
    case PLAYER1:
        ...
        else if (IsKeyDown(KEY_S)) crounch(&personagem[PLAYER1], CROUNCH, CROUNCHING);
    ...
    case PLAYER2:
        ...
        else if (IsKeyDown(KEY_DOWN)) crounch(&personagem[PLAYER2], CROUNCH, CROUNCHING);
...
void inputWalk(personagens* personagem, int qual){
    ...
    {
    case PLAYER1:
        printf("%d\n", personagem[PLAYER1].facing);
        if(IsKeyDown(KEY_D) && IsKeyDown(KEY_W) && personagem[PLAYER1].facing == 1) jumpf(&personagem[PLAYER1], JUMP, JUMPINGF);
        else if(IsKeyDown(KEY_A) && IsKeyDown(KEY_W) && personagem[PLAYER1].facing == -1) jumpf(&personagem[PLAYER1], JUMP, JUMPINGF);
        else if(IsKeyDown(KEY_D) && IsKeyDown(KEY_T) && personagem[PLAYER1].facing == 1) attack(&personagem[PLAYER1], ATTACK, PUNCHING1F);
        else if(IsKeyDown(KEY_A) && IsKeyDown(KEY_W) && personagem[PLAYER1].facing == -1) attack(&personagem[PLAYER1], ATTACK, PUNCHING1F);

        else if(IsKeyDown(KEY_M)) attack(&personagem[PLAYER1], ATTACK, PUNCHING1F);
...
void inputAttack(personagens* personagem, int qual){
    ...
    {
    case PLAYER1:
        if(IsKeyDown(KEY_T) && personagem[PLAYER1].animationSubState != PUNCHING1 && !(IsKeyDown(KEY_A) || IsKeyDown(KEY_D))) attack(&personagem[PLAYER1], ATTACK, PUNCHING1);
...
void inputCrounch(personagens* personagem, int qual){
    switch (qual)
    {
    case PLAYER1:
        if(IsKeyUp(KEY_S)) chstates(&personagem[PLAYER1], IDLE, IDLING);
        break;
    case PLAYER2:
        if(IsKeyUp(KEY_DOWN)) chstates(&personagem[PLAYER2], IDLE, IDLING);
        break;
    default:
        break;
    }
...
void handleInput( bool* can_update_animation, enum animStates* animState, personagens* personagem){
    ...
    for(int i = PLAYER1; i <= PLAYER2; i++){
        ...
        case JUMP:
            ...
            inputJump(personagem, i);
            personagem[i].layer = 2;
            break;
        case CROUNCH:
            onCrounching(&personagem[i]);
            inputCrounch(personagem, i);
            break;
        case ATTACK:
