   ...
   // Configurações iniciais
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        for(int i = PLAYER1; i <= PLAYER2; i++){
            ...
            if(entidades[i].animationState == CROUNCH){ 
                entidades[i].pushbox.height /= 2;
                entidades[i].pushbox.y += entidades[i].pushbox.height;
            }
        }
        ...
        switch (entidades[PLAYER1].animations[entidades[PLAYER1].animationSubState].cur)
        {
        case 0:
            cor = GREEN;
            break;
